...
bool AbstractWindowInterface::isFullScreenWindow(const QRect &wGeometry) const
{
    ...
    for (const auto scr : qGuiApp->screens()) {
        const auto screenGeometry = scr->geometry();
...
bool AbstractWindowInterface::isSidepanel(const QRect &wGeometry) const
{
    ...
    for (const auto scr : qGuiApp->screens()) {
        const auto curScrGeometry = scr->geometry();
...
{
    // Use org.freedesktop.DBus.Properties.Get to check availability and read
    // property.  asyncCall() avoids blocking the event loop — the reply is
    ...
    if (!m_isKWinInterfaceAvailable) {
        ...
        connect(watcher, &QDBusPendingCallWatcher::finished, this,
            ...
            if (!reply.arguments().isEmpty()) {
                ...
                m_isVirtualDesktopNavigationWrappingAround =
